ScoutGlobals = {"clickable": false}
$(document).ready(function () {
    function onStart () {
        $(".i_scouts_content_actions").hide();
        tableRowClickable(false);
    }
    onStart();


    $(document).on('focusout', '.td_scouts_content_name', function () {
        if ($(this).text().trim() != "") {
            triggerAjax(`rename_scout("${$(this).parent().find(":first-child").html()}", "${getElementByIndex("td_scouts_content_name", getIndexOfElement($(this))).text().trim()}")`, true, '/scouts_process', renameCB, {});
        }
    });
    ...
    $(document).on("click", ".button_scouts_content_actions", function () {
        let buttonId = getIndexOfElement($(this));
        $("#div_scouts_content_sleep").hide()
        $(".i_scouts_content_actions").hide();
        $(".i_scouts_content").hide();
        if (!ScoutGlobals["clickable"]) {
            tableRowClickable(true);
            getElementByIndex("i_scouts_content_actions", getIndexOfElement($(this))).show();
            if (buttonId == 4) {
                $("#div_scouts_content_sleep").show();
            }
            $(document).off("click", "#table_scouts_content tr");
            $(document).on("click", "#table_scouts_content tr", function() {
                let scoutId = $(this).find(":first-child").html();
                let rowId = getIndexOfElement($(this))
                if (rowId != -1) {
                    $(".button_scouts_content_actions").prop('disabled', true);
                    getElementByIndex("i_scouts_content", rowId).show();
                    if (buttonId == 0) {
                        triggerAjax(`bridge_scout("${scoutId}")`, true, "/scouts_process", bridgeCB, undefined)
                    } else if (buttonId == 1) {
                        triggerAjax(`disconnect_scout("${scoutId}")`, true, "/scouts_process", standardScoutFunctionCB, undefined)
                    } else if (buttonId == 2) {
                        triggerAjax(`kill_scout("${scoutId}")`, true, "/scouts_process", standardScoutFunctionCB, undefined)
                    } else if (buttonId == 3) {
                        triggerAjax(`ping_scout("${scoutId}")`, true, "/scouts_process", standardScoutFunctionCB, undefined)
                    } else if (buttonId == 4) {
                        triggerAjax(`sleep_scout("${scoutId}", "${$("#input_scouts_content_sleep").val()}")`, true, "/scouts_process", standardScoutFunctionCB, undefined)
                    }
                }
            });
        } else {
    ...
    function standardScoutFunctionCB(recvData) {
        let status = recvData["status"];
        let message = recvData["message"];
        tableRowClickable(false);
        $(".i_scouts_content_actions").hide();
        ...
        $(".button_scouts_content_actions").prop('disabled', false);
        $(document).off("click", "#table_scouts_content tr")
    }
    function bridgeCB(recvData) {
        let status = recvData["status"];
        let message = recvData["message"];
        if (status == "ok") {
            window.location.href = document.location.origin + "/direct"
        } else {
            show_message(message, true)
        }
    }
...
function tableRowClickable(clickable){
    if(clickable) {
        $("#table_scouts_content tr").not(":first").each(function() {
            $(this).addClass('clickable');
        })
        ScoutGlobals["clickable"] = true
    }
    else {
        $("#table_scouts_content tr").not(":first").each(function() {
